// https://github.com/zfigura/semblance/blob/master/src/x86_instr.c
/*
...
static void print_arg(char *_ip, struct instr *instr, int i, int bits) {
    ...
    case XM:
        ...
        if (asm_syntax == GAS) {
            ...
            if (instr->modrm_disp == DISP_8) {
                ...
                int32_t svalue = (int32_t) value;
                if (instr->modrm_reg == -1 && !has_sib)
                    sprintf(out+strlen(out), "%08xh", (unsigned int)value);   /* absolute memory is unsigned */
                else if (svalue < 0)
...
unsigned int disasm(const unsigned char *_bytes, unsigned int max, int offset, char *output) {
	...
	if (max > sizeof (bytes))
		max = sizeof (bytes);
	memcpy (bytes, (const void *)_bytes, max);
	memset (&instr, 0, sizeof (instr));
	len = get_instr (offset, bytes, &instr, 16);
	if (len < 1)
		return -1;
	sprint_instr (output, bytes, max, &instr, 16);
	return len;
}
