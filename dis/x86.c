...
enum {
	...
	IMM8, IMM16, IMM32, REL8, REL16, REL32, PTR1632, R, RM,
...
static INSTRUCTION HUGE standard_instructions[256] = {
	{ 1, BYTE, "add ", 2, RM, R }, // 0
...
static INSTRUCTION HUGE extended_instructions[256] = {
	{ 1, WORD, "sldt ", 1, RM }, // 0
	{ 1, 0, "sgdtd ", 1, RM }, // 1
...
unsigned int disasm(const unsigned char *__bytes, unsigned int max, unsigned long offset, char *output) {
	...
	if (max > sizeof (pad))
		max = sizeof (pad);
	memcpy (pad, __bytes, max);
	bytes = (unsigned char *)pad;
	base = bytes;
	opcode = *bytes;
	bytes++;
...
return -1;
	...
	modRM_mod = ((*bytes) >> 6) & 3; // 3; // Bits 7-6.
	modRM_reg = ((*bytes) >> 3) & 7; // 7; // Bits 5-3.
	modRM_rm = (*bytes++) & 7; // 7; // Bits 2-0.
	...
	if (modRM_mod == 3) { // Register addressing mode.
		switch (instructions[opcode].size) {
			case BYTE:
	...
	} else if (modRM_mod == 0 && modRM_rm == 5) { // Displacement only addressing mode.
		sprintf(RM_output, "[0x%x]", *(int *)bytes);
		bytes += 4;
	} else { // One-byte or four-byte signed displacement follows addressing mode byte(s).
		if (modRM_rm == 4) { // Contains SIB byte
			char SIB_scale = ((*bytes) >> 6) & 3; // Bits 7-6.
			char SIB_index = ((*bytes) >> 3) & 7; // Bits 5-3.
			char SIB_base = (*bytes++) & 7; // Bits 2-0.
		...
		if (modRM_mod == 1) { // One-byte signed displacement follows addressing mode byte(s).
			if (*bytes > 0x7F) {
				sprintf(RM_output + strlen(RM_output), "-0x%x]", -*(char *)bytes++);
			} else {
				sprintf(RM_output + strlen(RM_output), "+0x%x]", *(char *)bytes++);
			}
		} else if (modRM_mod == 2) { // Four-byte signed displacement follows addressing mode byte(s).
...
OUTPUT:
	...
	for (i = 0; i < instructions[opcode].argument_count; i++) {
		if (i > 0) {
			strcat(output, ",");
		}
